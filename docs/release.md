...
ყოველი რელიზი ორ ინსტალერს შეიცავს (Velopack-ის ორი არხი):

| არხი | ფაილი | რას შეიცავს |
|---|---|---|
| `win` | `Stenor-win-Setup.exe` | self-contained — .NET-იც შიგნითაა, არაფერი სჭირდება (დაინსტალირებული ~317 MB) |
| `win-lite` | `Stenor-win-lite-Setup.exe` | framework-dependent — მხოლოდ აპი (~19 MB ინსტალერი, ~40 MB დაინსტალირებული); სჭირდება **.NET 10 Desktop Runtime (x64)** — თუ არ უდგას, ინსტალერი თავად შესთავაზებს ჩამოტვირთვას |

არხი პაკეტშივეა ჩაწერილი, ამიტომ განახლებებსაც თითოეული ინსტალაცია თავისი არხიდან
იღებს — lite ვერსია lite-ად რჩება.
...
3. **აწყობა** — აქვეყნებს აპს და კრავს ორივე ინსტალერს:

   ```powershell
   pwsh scripts/pack.ps1
   ```

   შედეგი `releases/`-ში, თითო კომპლექტი არხზე: `Stenor-win-Setup.exe` /
   `Stenor-win-lite-Setup.exe`, `Stenor-X.Y.Z-full.nupkg` /
   `Stenor-X.Y.Z-win-lite-full.nupkg` (+ `-delta.nupkg`, თუ მე-2 ნაბიჯი გაიარე),
   `releases.win.json` / `releases.win-lite.json` და Portable zip-ები.
...
5. **GitHub Release-ზე ატვირთვა** — ორი გამოძახება (თითო არხზე) ერთსა და იმავე
   რელიზში ერთიანდება `--merge`-ით; ამის შემდეგ დაინსტალირებული აპები თავად განახლდებიან:
